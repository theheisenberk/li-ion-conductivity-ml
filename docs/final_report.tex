...
The dataset is derived from the \href{https://github.com/NRC-Mila/OBELiX}{OBELiX}
dataset and consists of 478 training samples and 121 held-out test samples of
...
The target spans roughly 20~orders of magnitude ($10^{-20}$ to
...
and SMACT baseline (all with sklearn defaults, no Optuna).  Table values now
report both cross-validation and held-out test performance for ranking and
explained variance:
...
\begin{center}
      ...
      \begin{tabular}{lcc|cc}
            ...
            Embedding       & $\Rtwo$                                        & $\rhoS$                               & $\Rtwo$ & $\rhoS$ \\
            \midrule
            Mat2Vec (200-d) & .713                                           & .876                                  & .265    & .622    \\
            Magpie (22-d)   & .694                                           & .856                                  & .398    & .656    \\
            MegNet16 (16-d) & .659                                           & .861                                  & .390    & .644    \\
            All combined    & .700                                           & .872                                  & .145    & .589    \\
            \bottomrule
      \end{tabular}
\end{center}

While Mat2Vec has the highest CV $\Rtwo$, Magpie gives the strongest held-out
test ranking quality among the compact embeddings (test $\rhoS = 0.656$),
which supports its use as the Stage~0 representative descriptor.

The choice of Magpie was deliberate and grounded in the nature of the
embeddings:
...
\begin{center}
      ...
      \begin{tabular}{lp{5cm}rcc|cc}
            \toprule
                                           &                                                                                                    &         & \multicolumn{2}{c|}{\textbf{Cross-Validation}} & \multicolumn{2}{c}{\textbf{Test Set}}                     \\
            Experiment                     & Features included                                                                                  & \#Feat. & $\Rtwo$                                        & $\rhoS$                               & $\Rtwo$ & $\rhoS$ \\
            \midrule
            \texttt{stage0\_magpie}        & Baseline (composition only)                                                                        & 129     & .739                                           & .872                                  & .458    & .691    \\
            \texttt{stage1\_basic\_struct} & + density, volume/atom, $Z$, space-group number                                                    & 133     & .737                                           & .880                                  & .468    & .716    \\
            \texttt{stage1\_geometry}      & + lattice parameters ($a,b,c,\alpha,\beta,\gamma$), CIF-derived Li-site metrics, anisotropy ratios & 152     & .737                                           & .887                                  & .556    & .711    \\
            \texttt{stage1\_full\_struct}  & + 230 space-group one-hot columns                                                                  & 382     & .737                                           & .890                                  & .591    & .701    \\
            \bottomrule
      \end{tabular}
      \setlength{\tabcolsep}{6pt}
\end{center}

The CV $\Rtwo$ values are nearly identical across all four variants, while
ranking quality (CV $\rhoS$) improves progressively with richer structural
features. On the test set, richer structure improves $\Rtwo$, but the best
test ranking metric in this block is achieved by \texttt{stage1\_basic\_struct}
(test $\rhoS = 0.7157$), with \texttt{stage1\_geometry} very close
($\rhoS = 0.7114$).

Crucially, even though \texttt{stage1\_basic\_struct} has a slightly better Spearman
$\rho$, \texttt{stage1\_geometry} exhibits a significant increase of approximately
10\% in $\Rtwo$. This is important given the underlying correlation between $\Rtwo$
and $\rhoS$. While explicit runs demonstrating this are not included here, moving
forward with multiple models revealed that improving $\Rtwo$ at this stage proved
useful later, even if it did not change the predicted ordering immediately.
Conceptually, this stage improved the continuous precision of the model, bringing
it closer to the threshold required for the discrete Spearman rank correlation to
improve in subsequent stages.
...
382~features) increases test $\Rtwo$ but does \emph{not} improve held-out
ranking quality versus geometry-only features (test $\rhoS$: 0.7008 vs 0.7114).
This is unsurprising: the lattice parameters
...
additional robust ranking signal beyond what the continuous lattice descriptors
...
It uses sklearn default hyperparameters and provides the best balance of
predictive strength, descriptor richness, and transferability into Stage~2.
...
\begin{enumerate}[leftmargin=*]
      \item \textbf{Gradient-based learners require a differentiable loss.}
            ...
            squared-error loss. The Spearman correlation involves a rank transformation,
            which is a piecewise-constant, non-smooth function of the predictions.
            Optimising for it directly would require either surrogate-gradient tricks,
            a custom loss function with non-trivial sub-gradient computation, or a
            fully black-box wrapper (e.g.\ evolutionary strategies via Optuna).
            These approaches are less sample-efficient, and substantially more
            complex to implement and validate than the smooth RMSE objective used here.
...
\subsection{Feature Importance of the Final Stage~2 Model}

To interpret the final representative model, permutation feature importance was
computed on the Stage~2 physics model using the full Stage~2 feature set.
Figure~\ref{fig:final_stage2_feat_imp} shows the top features (green =
physics-informed, red = Stage~1 geometry/structure, blue = Stage~0
composition).

\begin{figure}[H]
      \centering
      \includegraphics[width=0.95\textwidth]{final_stage2_physics_feature_importance.png}
      \caption{Permutation feature importance for the final Stage~2 physics model.
            Stage-aware color coding is used: blue = Stage~0 composition features,
            red = Stage~1 geometry/structure features, and green = Stage~2
            physics-informed features.}
      \label{fig:final_stage2_feat_imp}
\end{figure}

Comparing Figure~\ref{fig:feat_imp} (Stage~1 geometry model) with
Figure~\ref{fig:final_stage2_feat_imp} (final Stage~2 physics model) shows a
clear ranking shift.  In Stage~1, \texttt{lattice\_c} was the top feature
(0.203) and \texttt{sigma\_is\_coerced} was second (0.156).  In the final
Stage~2 model, \texttt{sigma\_is\_coerced} rises to first (0.267), followed
by \texttt{lattice\_c} (0.244) and \texttt{ewald\_energy\_avg} (0.226).

This change is consistent with the modelling setup.  The
\texttt{sigma\_is\_coerced} flag marks censored target values (thresholded
conductivities such as ``$<10^{-10}$''), which represent a distinct
measurement-uncertainty regime.  After adding sparse but high-impact
physics-informed descriptors, the boosted trees gain stronger interaction
structure and use \texttt{sigma\_is\_coerced} more aggressively as an early
partitioning variable, increasing its permutation importance.

Needless to say, this does not mean that \texttt{sigma\_is\_coerced} is a physical transport
descriptor.  Instead, it functions as a data-quality/context indicator that
helps the model treat censored measurements appropriately before relying on
downstream feature interactions.  The principal structure--property signal for
the Stage~2 ranking improvement still comes from physically meaningful
Stage~1/Stage~2 descriptors such as \texttt{lattice\_c},
\texttt{ewald\_energy\_avg}, and \texttt{li\_voronoi\_cn\_avg}.
...
\begin{figure}[H]
      ...
      \end{subfigure}
      \caption{Pairwise overlays of consecutive stages to illustrate the incremental improvements at each stage.}
      \label{fig:pairwise}
\end{figure}
...
For direct comparability, the corresponding hyperparameter sets are listed
below:
...
\begin{center}
      ...
      \begin{tabular}{lccc}
            \toprule
            Parameter                   & sklearn defaults & Optuna (depth 3--8) & Optuna (depth 3--30) \\
            \midrule
            \texttt{max\_depth}         & None             & 5                   & 3                    \\
            \texttt{learning\_rate}     & 0.100            & 0.0404              & 0.0445               \\
            \texttt{max\_leaf\_nodes}   & 31               & 89                  & 15                   \\
            \texttt{min\_samples\_leaf} & 20               & 15                  & 15                   \\
            \texttt{l2\_regularization} & 0                & $8.93\times10^{-4}$ & $9.41\times10^{-5}$  \\
            \texttt{max\_bins}          & 255              & 64                  & 125                  \\
            \texttt{max\_iter}          & 100              & 76                  & 138                  \\
            \bottomrule
      \end{tabular}
\end{center}

These hyperparameters make the failure mode explicit: both Optuna runs move
toward more constrained trees than the defaults (especially the expanded run,
which collapses to \texttt{max\_depth=3} and \texttt{max\_leaf\_nodes=15}).
With the gap-penalised objective, this is rewarded because it reduces the
train--validation gap, even when it suppresses useful model capacity and lowers
held-out ranking quality.  In this Stage~1 setting, $\lambda=0.2$ therefore
acts as over-regularisation rather than true generalisation control.
...
Why do these physics features fail to be computed even when a valid CIF is present? The root causes are deeply tied to the structural complexity of many advanced lithium-ion conductors, which frequently exhibit partial occupancies, mixed crystal sites, and complex bonding:

\begin{itemize}[leftmargin=*]
      \item \textbf{Bond-Valence (BV) Mismatch:} Requires unequivocally identifying known anion neighbours (O, S, F, Cl, Br, I, N, Se) around the Li site to apply tabulated bond-valence parameters ($R_0$). In highly disordered structures (e.g., argyrodites), mixed anion sites confuse neighbour identification. Furthermore, in non-anionic Zintl phases (e.g., Li$_8$GeP$_4$), Li has no valid anionic neighbours, rendering the calculation impossible.
      \item \textbf{Ewald Site Energy:} Relies on the assignment of unambiguous integer oxidation states that strictly balance the unit cell charge. Fractional compositions and mixed site occupancies often prevent algorithms from finding a consistent, charge-balanced set of oxidation states, which strictly precludes any Ewald electrostatic summation.
      \item \textbf{Voronoi Coordination Number:} Demands a cleanly converging geometric tessellation around the mobile ions. When partial occupancies drop to very low fractional values (e.g., 2.5\% occupied Li sites in some NASICONs), symmetric expansion generates a dense ``cloud'' of nearly overlapping, sparsely occupied positions. These near-degenerate coordinate points create pathological Voronoi polyhedra with zero-area faces, causing the geometric analysis to fail completely.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
      ...
      \item \textbf{Ranking uncertainty quantification}: quantile regression
            models were explored during development to produce pointwise
            prediction intervals, but were ultimately excluded from this
            report---the ranking-focused objective (Spearman~$\rhoS$) is not
            naturally expressed through pointwise confidence bands.  Future work
            could instead quantify ranking uncertainty via bootstrap resampling
            of~$\rhoS$ or pairwise rank-inversion probabilities.
\end{itemize}
