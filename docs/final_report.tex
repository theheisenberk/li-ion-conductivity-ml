...
\title{%
  \textbf{Final Report: Machine-Learning Prediction of\\
  Lithium-Ion Conductivity from Composition,\\
  Structure, and Physics-Informed Features}
}
\author{Berk Oguz}
\date{February 2026}
...
proceed to expensive DFT calculations or synthesis.  For this reason,
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Stage~0 (Composition-only):} elemental ratios, SMACT
  ...
  \item \textbf{Stage~1 (+ Structural geometry):} lattice parameters, density,
        Li-site environment metrics extracted from CIF files and CSV metadata.
  \item \textbf{Stage~2 (+ Physics-informed):} bond-valence mismatch, Ewald
        site energy, and Voronoi coordination number computed with
        \texttt{pymatgen}.
\end{enumerate}
...
TPE sampler, 50 trials) using 5-fold cross-validation with a
\emph{generalization penalty} to suppress overfitting.
...
The target spans roughly 20~orders of magnitude ($10^{-30}$ to
...
(identifiers, DOIs, reference fields) are placed in a mandatory exclusion list
...
and SMACT baseline (all with sklearn defaults, no Optuna):
...
Stage~0 model achieves $\Rtwo_{\text{CV}} = 0.746$, confirming that the
...
\begin{tabular}{lrccc}
  \toprule
  Experiment & \#Feat. & CV $\Rtwo$ & CV RMSE \\
  \midrule
  \texttt{stage0\_magpie}      & 129 & 0.7393 & 1.3675 \\
  \texttt{stage1\_basic\_struct}  & 133 & 0.7369 & 1.3737 \\
  \texttt{stage1\_geometry}    & 152 & 0.7369 & 1.3737 \\
  \texttt{stage1\_full\_struct}   & 382 & 0.7370 & 1.3734 \\
  \bottomrule
\end{tabular}
\end{center}
...
objective that evaluates $\rhoS$ on OOF predictions).  Within the time
...
\begin{tabular}{lcc|cc}
  ...
  physics (w/ SG one-hot) & .740 & .886 & .598 & .706 \\
...
\subsection{Side-by-Side Parity Plots (Triptych)}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Triptych parity plots for the three stages.  Each panel shares
  identical axis limits for direct visual comparison.  The diagonal dashed line
