...
physics-informed, blue = composition/geometry).
...
\begin{figure}[H]
      ...
      Physics-informed descriptors are shown in green and composition/geometry
      descriptors in blue.}
      \label{fig:final_stage2_feat_imp}
\end{figure}

The strongest contributors are \texttt{sigma\_is\_coerced} (0.267),
\texttt{lattice\_c} (0.244), and \texttt{ewald\_energy\_avg} (0.226),
followed by embedding features and Li-site descriptors such as
\texttt{li\_voronoi\_cn\_avg}.  This profile supports the Stage~2 results:
physics-informed terms provide substantial predictive signal while structural
geometry remains essential, explaining why Stage~2 yields the best held-out
ranking performance.

The prominence of \texttt{sigma\_is\_coerced} is expected in this pipeline:
it marks samples where the original conductivity was reported as a thresholded
value (e.g., ``$<10^{-10}$'') and then coerced during target construction.
These censored measurements follow a different uncertainty regime, so the
tree-based model uses this flag as an early split to separate them from
precisely measured values.  Therefore, its high importance should be read as a
measurement-context effect rather than a model discrepancy.
